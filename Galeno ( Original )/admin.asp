<% if request.form("Comportamiento") = "true" then
	...
	xVariable= request.Form("txt_SINO")
      ...
      sqlvalida = " SELECT * " & _
              " FROM registro" & _
              " WHERE Trim(Ucase(email)) = '" & Trim(Ucase(xemail)) & "'"


      Set rs = Server.CreateObject("ADODB.Recordset")
      rs.Open sqlvalida, cn, 3, 3 

      response.write "xemail:"&xemail
      response.end 
...
<body>
<FORM action="Registro.asp" method="post" name="frmReg" onSubmit="MM_validateForm('txt_Nombre','','R','txt_Apellido','','R','txt_Especialidad','','R','txt_Sub','','R','txt_consult','','R','txt_Hosp','','RisNum','txt_Telefono1','','R','txt_email','','RisEmail');return document.MM_returnValue">
